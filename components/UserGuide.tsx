...
const SectionTitle = ({ title, subtitle }: { title: string, subtitle: string }) => (
    <div className="mb-8 border-b border-white/20 pb-4">
        <h1 className="text-3xl font-black text-slate-800 dark:text-white mb-2">{title}</h1>
...
const IconDefinition = ({ icon: Icon, label, desc, color = "text-indigo-500" }: { icon: any, label: string, desc: string, color?: string }) => (
    <div className="flex items-start gap-4 p-4 rounded-2xl glass-card border border-white/50 shadow-sm hover:shadow-md transition-all hover:scale-[1.02]">
        <div className={`w-12 h-12 rounded-xl bg-white/50 dark:bg-white/10 flex items-center justify-center shrink-0 ${color} shadow-inner`}>
            <Icon className="w-6 h-6" />
        </div>
        <div>
            <h4 className="font-black text-slate-800 dark:text-white text-sm mb-1">{label}</h4>
...
const FaqItem = ({ question, answer }: { question: string, answer: string }) => {
    ...
    return (
        <div className="border border-white/30 rounded-2xl overflow-hidden mb-3 bg-white/40 dark:bg-white/5 backdrop-blur-sm transition-all hover:bg-white/60">
            <button 
            ...
            {isOpen && (
                <div className="p-4 pt-0 text-sm text-slate-600 dark:text-white/70 leading-relaxed border-t border-white/20">
                    {answer}
                </div>
            )}
        </div>
...
const NavButton = ({ id, label, icon: Icon, activeTab, onClick }: { id: string, label: string, icon: any, activeTab: string, onClick: (id: string) => void }) => (
    <button 
        onClick={() => onClick(id)}
        className={`
            w-full text-right p-3.5 rounded-xl flex items-center gap-3 transition-all duration-200 group relative overflow-hidden mb-1
            ${activeTab === id 
...
const UserGuide: React.FC = () => {
    ...
    const renderContent = () => {
        switch (activeTab) {
                ...
                return (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                            <div className="glass-card p-6 rounded-3xl border border-indigo-100 dark:border-white/5 shadow-sm relative overflow-hidden group hover:-translate-y-1 transition-all bg-indigo-50/50">
                                <div className="absolute -right-6 -top-6 w-24 h-24 bg-indigo-100/50 rounded-full group-hover:scale-125 transition-transform"></div>
                                <span className="relative font-black text-4xl text-indigo-200 mb-2 block">01</span>
                            ...
                            <div className="glass-card p-6 rounded-3xl border border-emerald-100 dark:border-white/5 shadow-sm relative overflow-hidden group hover:-translate-y-1 transition-all bg-emerald-50/50">
                                <div className="absolute -right-6 -top-6 w-24 h-24 bg-emerald-100/50 rounded-full group-hover:scale-125 transition-transform"></div>
                                <span className="relative font-black text-4xl text-emerald-200 mb-2 block">02</span>
                            ...
                            <div className="glass-card p-6 rounded-3xl border border-amber-100 dark:border-white/5 shadow-sm relative overflow-hidden group hover:-translate-y-1 transition-all bg-amber-50/50">
                                <div className="absolute -right-6 -top-6 w-24 h-24 bg-amber-100/50 rounded-full group-hover:scale-125 transition-transform"></div>
                                <span className="relative font-black text-4xl text-amber-200 mb-2 block">03</span>
                        ...
                        <div className="bg-white/40 dark:bg-white/5 rounded-2xl p-6 border border-white/40 backdrop-blur-md">
            ...
            case 'about':
                return (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col items-center justify-center min-h-[50vh] pb-20">
                        <div className="w-32 h-32 bg-white/20 backdrop-blur-md rounded-full shadow-2xl flex items-center justify-center mb-6 overflow-hidden border-4 border-white/30 p-1">
                            <BrandLogo className="w-full h-full" showText={false} />
                        </div>
                        <h1 className="text-4xl font-black text-slate-800 dark:text-white mb-2">تطبيق راصد</h1>
                        <p className="text-slate-500 dark:text-white/60 font-bold mb-8 bg-white/30 px-4 py-1 rounded-full text-xs border border-white/20">الإصدار 3.4.0</p>
                        
                        <div className="glass-card border border-white/50 rounded-3xl p-8 max-w-md w-full text-center shadow-lg">
                            <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-6">فريق العمل</h2>
                            
                            <div className="flex flex-col gap-6">
                                <div className="flex items-center gap-4 p-4 bg-white/40 dark:bg-white/5 rounded-2xl border border-white/40">
                                    <div className="w-12 h-12 bg-indigo-100/50 rounded-full flex items-center justify-center text-indigo-600 dark:text-indigo-400 shadow-inner">
                                ...
                                <div className="flex items-center gap-4 p-4 bg-white/40 dark:bg-white/5 rounded-2xl border border-white/40">
                                    <div className="w-12 h-12 bg-emerald-100/50 rounded-full flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-inner">
    ...
    const handleHTMLExport = () => {
        ...
        const htmlContent = `<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>دليل راصد</title><style>body { font-family: sans-serif; padding: 40px; line-height: 1.6; } h1, h2, h3 { color: #1e293b; } p { color: #475569; } .glass-card { background-color: #f8fafc; padding: 20px; border-radius: 10px; margin: 20px 0; border: 1px solid #e2e8f0; }</style></head><body>${rawHTML}</body></html>`;
    ...
    return (
        <div className="flex h-full bg-transparent overflow-hidden" dir="rtl">
            <aside className="w-64 bg-white/20 dark:bg-black/20 border-l border-white/20 flex flex-col hidden md:flex print:hidden shrink-0 z-20 shadow-sm relative backdrop-blur-md">
                ...
                <div className="p-4 border-t border-white/10 bg-white/10">
                    <button onClick={handleHTMLExport} className="w-full bg-white/40 dark:bg-white/5 text-slate-700 dark:text-white py-2.5 rounded-xl mb-2 hover:bg-white/60 border border-white/20 text-xs font-bold flex items-center justify-center gap-2 transition-all"><Download className="w-4 h-4" /> حفظ HTML</button>
                    <button onClick={() => window.print()} className="w-full bg-slate-800 text-white py-2.5 rounded-xl hover:bg-slate-900 text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-lg"><Printer className="w-4 h-4" /> طباعة</button>
                </div>
            </aside>
            <main className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12 text-gray-800 dark:text-gray-100 custom-scrollbar relative" id="user-guide-content">
                <div className="max-w-4xl mx-auto">
                    <div className="md:hidden mb-6 p-4 bg-blue-50/50 rounded-xl text-center border border-blue-100 print:hidden"><p className="text-sm font-bold text-blue-800">للتصفح الكامل، يفضل استخدام جهاز أكبر.</p><div className="flex flex-wrap gap-2 justify-center mt-3">{['icons', 'intro', 'about'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={`px-3 py-1 rounded-lg text-xs font-bold ${activeTab === tab ? 'bg-primary text-white' : 'bg-white/40 text-slate-600'}`}>{tab === 'icons' ? 'الرموز' : tab === 'intro' ? 'البداية' : 'حول'}</button>))}</div></div>
                    {renderContent()}
                    <div className="mt-12 pt-8 border-t border-white/10 text-center print:hidden"><p className="text-xs font-bold text-slate-400 dark:text-white/30">جميع الحقوق محفوظة © {new Date().getFullYear()}</p></div>
                </div>
            </main>
