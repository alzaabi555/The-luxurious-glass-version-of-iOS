...
import { Student, ScheduleDay, PeriodTime, Group, AssessmentTool } from '../types';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [teacherInfo, setTeacherInfo] = useState<TeacherInfo>(() => {
    try { 
        return { 
            ...
            avatar: localStorage.getItem('teacherAvatar') || '' // Load avatar
        }; 
    } catch { return { name: '', school: '', subject: '', governorate: '', avatar: '' }; }
  });

  // Global Assessment Tools State
  ...
  useEffect(() => {
    ...
    if (teacherInfo.avatar) localStorage.setItem('teacherAvatar', teacherInfo.avatar); // Save avatar
  ...
  }, [students, classes, groups, teacherInfo, schedule, periodTimes, currentSemester, assessmentTools]);
